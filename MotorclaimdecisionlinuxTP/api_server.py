...
# Daily transaction log file - logs all API requests for TP
...
TRANSACTION_LOG_FILE = os.path.join(UNIFIED_LOG_DIR, f"api_transactions_tp_{current_date}.log")
...
def log_request_info():
    ...
    if current_date != _last_log_date:
        ...
        new_log_file = os.path.join(UNIFIED_LOG_DIR, f"api_transactions_tp_{current_date}.log")
    ...
    transaction_logger.info(
        f"TP | REQUEST | {method} | {path} | IP: {client_ip} | User-Agent: {user_agent[:100]}"
    )
...
def log_response_info(response):
    ...
    # Log response
    transaction_logger.info(
        f"TP | RESPONSE | {method} | {path} | Status: {status_code} | Size: {response_size} bytes"
    )
    
    return response


# Error handlers for HTTP exceptions (404, 405, etc.) - don't log these as errors
@app.errorhandler(404)
def handle_not_found(e):
    """Handle 404 Not Found - don't log as error (normal for favicon, bots, etc.)"""
    return jsonify({"error": "Not Found", "message": "The requested URL was not found on the server"}), 404

@app.errorhandler(405)
def handle_method_not_allowed(e):
    """Handle 405 Method Not Allowed - don't log as error (normal for wrong HTTP method)"""
...
# Global error handler for actual server errors (500, 502, 503, etc.)
# Note: This should be registered AFTER specific error handlers
@app.errorhandler(Exception)
def handle_global_error(e):
    """Global error handler - logs only actual server errors, not normal HTTP errors"""
    # Don't log 404, 405, 400, 401, 403 - these are normal HTTP errors
    # Check if it's a werkzeug HTTPException (which includes 404, 405, etc.)
    from werkzeug.exceptions import HTTPException
    
    if isinstance(e, HTTPException):
        # This is a normal HTTP error (404, 405, etc.) - don't log, just return the error
        # Flask will use the specific handlers registered above
        code = e.code if hasattr(e, 'code') else 500
        # Only log if it's a server error (5xx)
        if code >= 500:
            error_msg = f"Server error ({code}): {str(e)}"
            error_traceback = traceback.format_exc()
            logger.error(f"{error_msg}\n{error_traceback}")
        # Return the HTTP error response
        return jsonify({"error": e.name if hasattr(e, 'name') else "Error", "message": str(e)}), code
    
    # Log actual server errors (non-HTTP exceptions)
...
def process_claim_simplified():
    ...
    try:
        ...
        # Get response fields configuration - ALWAYS read from claim_config.json
        ...
        logger.info(f"✓ Loaded response_fields configuration from claim_config.json")
        logger.info(f"  Enabled fields count: {len([k for k, v in response_fields_config.items() if v])}")
        ...
        def process_single_party(idx, party):
            ...
            try:
                ...
                # Keep index for sorting, will be removed later
                filtered_response = {"index": idx}  # Keep index for sorting
                for field_name, field_value in base_response.items():
                    if field_name in ["index", "error"]:
                        continue  # Skip index (already added) and error
                ...
                filtered_error_response = {"index": idx}  # Keep index for sorting
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_party):
                try:
                    ...
                    print(f"  ✅ Party {result['index'] + 1} completed: {result.get('Decision', 'PENDING')}")
                except Exception as e:
                    ...
                    completed_results[idx] = filtered_error_response
                    print(f"  ❌ Party {idx + 1} failed: {str(e)[:100]}")
        
        # Sort results by index to maintain order
        results = [completed_results[i] for i in sorted(completed_results.keys())]
        
        # Remove index field from final response (already filtered by configuration)
