...
class UnifiedClaimProcessor:
    ...
    def _load_make_model_mapping(self):
        ...
        try:
            if os.path.exists(self.make_model_mapping_file):
                ...
                # Build lookup dictionary (key: (make_upper, model_upper) -> license_type)
    ...
    def lookup_license_type_from_make_model(self, car_make: str, car_model: str) -> str:
            ...
            if hasattr(self, '_mapping_partial') and self._mapping_partial:
                if car_make_clean in self._mapping_partial:
                    for model_val, license_val in self._mapping_partial[car_make_clean]:
                        if (model_val == car_model_clean or 
                            car_model_clean in model_val or 
                            model_val in car_model_clean):
                            return license_val
        ...
        try:
            ...
            # Vectorized exact match (faster than iterrows)
            mask = (
            ...
            # Vectorized partial match (faster than iterrows)
            ...
            if mask_make.any():
                ...
                model_series = make_matches[najm_model_col].astype(str).str.strip().str.upper()
                model_match = (
                    (model_series == car_model_clean) |
                    (model_series.str.contains(car_model_clean, na=False, regex=False)) |
                    (pd.Series([car_model_clean] * len(model_series)).str.contains(model_series, na=False, regex=False))
                )
                if model_match.any():
                    license_type = make_matches[model_match].iloc[0][license_type_col]
...
Translation (Arabic parts only, keep English unchanged, use LD report terminology):"""
    ...
    def _validate_recovery_decision(self, current_party_idx: int, current_party_info: Dict[str, Any], 
        ...
        # Check current party's model_recovery (calculate using same logic as global section)
        ...
        # Check model_recovery condition (same logic as global section)
        ...
        current_model_recovery = current_has_model_recovery
        
        # Initialize recovery analysis for current party
        current_party_recovery_analysis = {
            "recovery_field": current_recovery,
            "has_recovery_field": current_recovery_upper in ["TRUE", "1", "YES", "Y","TRUE","True"] or current_recovery in ["True", "true","TRUE","True"],
            "model_recovery": current_model_recovery,
        ...
        # Rule 1: ACCEPTED_WITH_RECOVERY should only apply to parties with liability < 100% (not fully at fault)
        ...
        if current_recovery_upper in ["TRUE", "1", "YES", "Y","TRUE","True"] or current_recovery in ["True", "true","TRUE","True"]:
        ...
        if current_has_model_recovery:
            recovery_violations_found = True
            recovery_reasons.append(f"Current Party {current_party_idx + 1} has model_recovery=True/TRUE/true")
            current_party_recovery_analysis["violations_found"].append("model_recovery field is True/TRUE/true")
        ...
        if not recovery_violations_found and at_fault_parties:
            for at_fault_party in at_fault_parties:
                ...
                recovery_field = str(party_info.get("Recovery", "")).strip()
                recovery_field_upper = recovery_field.upper()
                if recovery_field_upper in ["TRUE", "1", "YES", "Y","TRUE","True"] or recovery_field in ["True", "true","TRUE","True"]:
                    recovery_violations_found = True
                    recovery_reasons.append(f"At-fault Party {at_fault_party['idx'] + 1} has Recovery=True/TRUE/true")
                    continue
                
                # Check model_recovery field (SAME AS Recovery logic) - calculate using same logic
                # Get License_Type_From_Make_Model from party_info, or fallback to party_raw_data or results
                other_license_type_make_model = str(party_info.get("License_Type_From_Make_Model", "")).strip()
                if not other_license_type_make_model and party_raw_data:
                    other_license_type_make_model = str(party_raw_data.get("License_Type_From_Make_Model", "")).strip()
                # Also check results dictionary if available (for parties already processed)
                if not other_license_type_make_model and at_fault_party['idx'] < len(all_parties):
                    # Try to get from results if available (parties processed in parallel might have it)
                    # Note: results might not be available yet, so we calculate on the fly
                    pass
                
                other_license_type_request = str(party_info.get("License_Type_From_Request", "")).strip()
                if not other_license_type_request and party_raw_data:
                    other_license_type_request = str(party_raw_data.get("License_Type_From_Request", "")).strip()
                ...
                if other_has_model_recovery:
                    recovery_violations_found = True
                    recovery_reasons.append(f"At-fault Party {at_fault_party['idx'] + 1} has model_recovery=True/TRUE/true")
                    continue
...
Translation (one line per phrase, format: original|translation):"""
            ...
            transaction_logger.info(
                f"OCR_TRANSLATION_REQUEST | "
                f"Model: {translation_model_to_use} | "
                f"Arabic_Phrases_Count: {len(phrases_to_translate)} | "
                f"Prompt_Length: {len(translate_prompt)} | "
                f"Phrases_Preview: {phrases_to_translate[:3] if phrases_to_translate else []}"
            )
            ...
            transaction_logger.info(
                f"OCR_TRANSLATION_RESPONSE | "
                f"Model: {translation_model_to_use} | "
                f"Status_Code: {response.status_code} | "
                f"Response_Length: {len(response.text) if response.text else 0}"
            )
            ...
            if response.status_code == 200:
                ...
                if translation_map:
                    ...
                    transaction_logger.info(
                        f"OCR_TRANSLATION_SUCCESS | "
                        f"Phrases_Translated: {len(translation_map)} | "
                        f"Original_Length: {len(ocr_text)} | "
                        f"Translated_Length: {len(result_text)} | "
                        f"Translation_Map: {json.dumps(translation_map, ensure_ascii=False)[:500]}"
                    )
                    print(f"  ✅ Translated {len(translation_map)} Arabic phrase(s), preserved report structure")
                    return result_text
                else:
                    transaction_logger.warning(
                        f"OCR_TRANSLATION_PARSE_FAILED | "
                        f"Could not parse translations from response | "
                        f"Response_Preview: {translated_text[:500]}"
                    )
                    print(f"  ⚠️ Could not parse translations, using original text")
                    return ocr_text
            else:
                transaction_logger.error(
                    f"OCR_TRANSLATION_FAILED | "
                    f"Status_Code: {response.status_code} | "
                    f"Response: {response.text[:500] if response.text else 'N/A'}"
                )
    ...
    def xml_to_json(self, xml_string: str) -> Dict[str, Any]:
        ...
        Handles "XML or text declaration not at start" errors by removing leading content
        """
        xml_clean = self.clean_data(xml_string)
        
        # Fix "XML or text declaration not at start" error by finding and removing content before XML declaration
        if '<?xml' in xml_clean:
            xml_decl_start = xml_clean.find('<?xml')
            if xml_decl_start > 0:
                # Remove everything before the XML declaration
                xml_clean = xml_clean[xml_decl_start:]
        ...
        try:
            ...
            error_msg = str(e)
            # Handle "XML or text declaration not at start" error
            if 'XML or text declaration not at start' in error_msg or 'declaration not at start' in error_msg.lower():
                # Find XML declaration and remove everything before it
                xml_decl_match = re.search(r'<\?xml[^>]*\?>', xml_clean)
                if xml_decl_match:
                    xml_clean = xml_clean[xml_decl_match.start():]
                else:
                    # If no XML declaration found, try to find first < tag
                    first_tag_match = re.search(r'<[^!?]', xml_clean)
                    if first_tag_match:
                        xml_clean = xml_clean[first_tag_match.start():]
                try:
                    root = ET.fromstring(xml_clean)
            ...
            elif 'unbound prefix' in error_msg or 'namespace' in error_msg.lower():
    ...
    def extract_party_info(self, party_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Get License_Type_From_Make_Model if available (added during pre-extraction)
        license_type_from_make_model = party_data.get("License_Type_From_Make_Model", "")
        ...
        return {
            ...
            "License_Type_From_Make_Model": str(license_type_from_make_model) if license_type_from_make_model else "",
    ...
    def _process_single_row(self, row_num: int, claim_data: Any, request_column: str, 
        ...
        PERFORMANCE: Reduced logging for large batches
        """
        results = []
        # Only log for small batches (< 20 rows) to improve performance
        verbose_logging = total_rows < 20
        if verbose_logging:
            ...
            try:
                ...
                if accident_info_raw:
                    ...
                    if isDAA_value is not None:
                        # Convert to string and normalize (handle True/False, "true"/"false", "TRUE"/"FALSE", 1/0)
                        ...
                        if isDAA_str in ['TRUE', '1', 'YES', 'Y', 'T']:
                            ...
                            daa_from_request['isDAA'] = isDAA_str if pd.notna(isDAA_value) else None
                    ...
                    # Log DAA extraction results (only for small batches)
                    if verbose_logging:
                        if any(daa_from_request.values()):
                            ...
                            print(f"  ℹ️ No DAA values found in Request data (Row {row_num + 1})")
            except Exception as e:
                error_msg = str(e)
                # Always log errors, but keep it brief
                if verbose_logging or total_rows < 100:
...
"""
                ...
                if decision == "ACCEPTED_WITH_RECOVERY":
                    # CRITICAL: Only validate ACCEPTED_WITH_RECOVERY for Tawuniya (التعاونية للتأمين) insured parties
                    current_insurance = str(party_info.get("Insurance_Name", "")).strip()
                    current_ic_english = str(party_info.get("ICEnglishName", "")).strip()
                    is_tawuniya_party = self._is_tawuniya_insurance(current_insurance, current_ic_english)
                    
                    if not is_tawuniya_party:
                        # Non-Tawuniya party with ACCEPTED_WITH_RECOVERY - downgrade to ACCEPTED
                        print(f"  ⚠️ VALIDATION: ACCEPTED_WITH_RECOVERY only applies to Tawuniya insured parties")
                        print(f"  ⚠️ Party is not insured with Tawuniya (Insurance: {current_insurance}, ICEnglishName: {current_ic_english})")
                        print(f"  ⚠️ Correcting decision from ACCEPTED_WITH_RECOVERY to ACCEPTED")
                        decision = "ACCEPTED"
                        ...
                        # Party is insured with Tawuniya - validate existing ACCEPTED_WITH_RECOVERY decision
                        if verbose_logging:
                            print(f"  ✅ Party is insured with Tawuniya - validating ACCEPTED_WITH_RECOVERY decision")
                    ...
                    # CRITICAL: Recovery validation applies when:
                    # 1. Current party is Tawuniya with liability > 0 AND (Recovery=True OR model_recovery=True), OR
                    # 2. Current party is Tawuniya with liability > 0 AND another Tawuniya party has Recovery=True/TRUE/true with liability > 0, OR
                    # 3. Current party is Tawuniya with liability > 0 AND another Tawuniya party has model_recovery=True/TRUE/true with liability > 0, OR
                    # 4. EXCEPTION: Current party is non-Tawuniya with liability < 100% AND another Tawuniya party has Recovery=True/TRUE/true with liability > 0, OR
                    # 5. EXCEPTION: Current party is non-Tawuniya with liability < 100% AND another Tawuniya party has model_recovery=True/TRUE/true with liability > 0
                    # NOTE: Non-Tawuniya parties SKIP recovery validation UNLESS exception applies (another Tawuniya party has Recovery/model_recovery=True with liability > 0)
                    
                    # Check if current party is insured with Tawuniya
                    current_insurance = str(party_info.get("Insurance_Name", "")).strip()
                    current_ic_english = str(party_info.get("ICEnglishName", "")).strip()
                    is_tawuniya_party = self._is_tawuniya_insurance(current_insurance, current_ic_english)
                    
                    # Only proceed with recovery validation if current party has liability < 100% (for recovery conditions)
                    if current_liability < 100:
                        # Check if any other party is Tawuniya with Recovery=True (any liability)
                        # If current party is non-Tawuniya with liability < 100%, upgrade to ACCEPTED_WITH_RECOVERY
                        other_tawuniya_with_recovery = False
                        other_tawuniya_party_info = None
                        
                        # Initialize other_tawuniya_with_model_recovery (will be set in loop if found)
                        other_tawuniya_with_model_recovery = False
                        other_tawuniya_model_recovery_party_info = None
                        ...
                        for other_idx, other_party_decision in enumerate(parties):
                            ...
                            # Fallback from decision
                            if other_party_info.get("Liability") == 0 and "liability" in other_party_decision:
                                other_party_info["Liability"] = int(other_party_decision.get("liability", 0))
                            
                            other_liability = other_party_info.get("Liability", 0)
                            other_insurance = str(other_party_info.get("Insurance_Name", "")).strip()
                            other_ic_english = str(other_party_info.get("ICEnglishName", "")).strip()
                            other_recovery = str(other_party_info.get("Recovery", "")).strip()
                            other_recovery_upper = other_recovery.upper()
                            
                            # Check if other party is Tawuniya with Recovery=True/TRUE/true AND liability > 0
                            # CRITICAL: For non-Tawuniya parties, if Tawuniya party has Recovery=True/TRUE/true with liability > 0, upgrade to ACCEPTED_WITH_RECOVERY
                            is_other_tawuniya = self._is_tawuniya_insurance(other_insurance, other_ic_english)
                            has_recovery = other_recovery_upper in ["TRUE", "1", "YES", "Y"] or other_recovery in ["True", "true"]
                            
                            # If Tawuniya party has Recovery=True AND liability > 0, trigger upgrade for non-Tawuniya parties
                            if is_other_tawuniya and has_recovery and other_liability > 0:
                                other_tawuniya_with_recovery = True
                                other_tawuniya_party_info = {
                                    "idx": other_idx,
                                    "insurance": other_insurance,
                                    "liability": other_liability,
                                    "recovery": other_recovery
                                }
                                # Don't break - continue to check for model_recovery as well
                            
                            # CRITICAL: Check if other party is Tawuniya with model_recovery=True/TRUE/true (SAME AS Recovery logic)
                            # Calculate model_recovery on the fly using same logic as global section
                            if is_other_tawuniya:
                                other_license_type_make_model = str(other_party_info.get("License_Type_From_Make_Model", "")).strip()
                                other_license_type_request = str(other_party_info.get("License_Type_From_Request", "")).strip()
                                ...
                                # If Tawuniya party has model_recovery=True AND liability > 0, trigger upgrade (SAME AS Recovery logic)
                                if other_has_model_recovery and other_liability > 0:
                                    other_tawuniya_with_model_recovery = True
                                    other_tawuniya_model_recovery_party_info = {
                                        "idx": other_idx,
                                        "insurance": other_insurance,
                                        "liability": other_liability,
                                        "license_type_make_model": other_license_type_make_model,
                                        "license_type_request": other_license_type_request
                                    }
                                    # Don't break - continue to check for Recovery as well
                                    
                                # Break if both Recovery and model_recovery found
                                if other_tawuniya_with_recovery and other_tawuniya_with_model_recovery:
                                    break
                        
                        # Proceed with recovery validation if:
                        # 1. Current party is Tawuniya with liability > 0 AND (Recovery=True/TRUE/true OR model_recovery=True/TRUE/true), OR
                        # 2. Current party is Tawuniya with liability > 0 AND another Tawuniya party has Recovery=True/TRUE/true with liability > 0, OR
                        # 3. Current party is Tawuniya with liability > 0 AND another Tawuniya party has model_recovery=True/TRUE/true with liability > 0, OR
                        # 4. EXCEPTION: Current party is non-Tawuniya with liability < 100% AND another Tawuniya party has Recovery=True/TRUE/true with liability > 0, OR
                        # 5. EXCEPTION: Current party is non-Tawuniya with liability < 100% AND another Tawuniya party has model_recovery=True/TRUE/true with liability > 0
                        # NOTE: Non-Tawuniya parties SKIP recovery validation UNLESS exception applies
                        should_validate_recovery = False
                        
                        # Check if current party has Recovery field = TRUE/True/true
                        current_recovery_field = str(party_info.get("Recovery", "")).strip()
                        ...
                        # Check if current party has model_recovery = True/TRUE/true
                        # First try to get from results (if already calculated)
                        current_model_recovery = False
                        if party_idx < len(results):
                            current_model_recovery = results[party_idx].get("model_recovery", False)
                        
                        # If not in results, calculate on the fly using same logic as global section
                        if not current_model_recovery:
                            current_license_type_make_model = str(party_info.get("License_Type_From_Make_Model", "")).strip()
                            current_license_type_request = str(party_info.get("License_Type_From_Request", "")).strip()
                            ...
                            current_request_mismatch = (current_license_type_request and 
                                                       current_license_type_request.lower() not in ["not identify", "not identified", "", "none", "nan", "null"] and
                                                       current_license_type_make_model.upper() != current_license_type_request.upper())
                            current_model_recovery = current_make_model_valid and (current_request_is_none_or_empty or current_request_mismatch)
                        
                        # Check if model_recovery is True/TRUE/true (boolean or string)
                        if isinstance(current_model_recovery, str):
                            current_model_recovery_str = str(current_model_recovery).strip()
                            current_has_model_recovery = current_model_recovery_str.upper() in ["TRUE", "1", "YES", "Y"] or current_model_recovery_str in ["True", "true"]
                        else:
                            current_has_model_recovery = bool(current_model_recovery)
                        
                        # Check if any other party has Recovery field = TRUE/True/true
                        other_party_with_recovery_field = False
                        other_recovery_field_party_info = None
                        
                        # Check if any other party has model_recovery = True/TRUE/true
                        other_party_with_model_recovery_field = False
                        other_model_recovery_field_party_info = None
                        
                        for check_idx, check_party_decision in enumerate(parties):
                            if check_idx == party_idx:
                                continue
                            check_party_raw = parties_data.get(check_idx, {})
                            if not check_party_raw and "party_info" in check_party_decision:
                                check_party_raw = check_party_decision.get("party_info", {})
                            check_party_info = self.extract_party_info(check_party_raw)
                            
                            # Check Recovery field
                            check_recovery_field = str(check_party_info.get("Recovery", "")).strip()
                            check_recovery_field_upper = check_recovery_field.upper()
                            if check_recovery_field_upper in ["TRUE", "1", "YES", "Y"] or check_recovery_field in ["True", "true","TRUE","True"]:
                                other_party_with_recovery_field = True
                                other_recovery_field_party_info = {
                                    "idx": check_idx,
                                    "recovery": check_recovery_field,
                                    "liability": check_party_info.get("Liability", 0)
                                }
                            
                            # Check model_recovery field (SAME AS Recovery logic)
                            # First try to get from results (if already calculated)
                            check_model_recovery = False
                            if check_idx < len(results):
                                check_model_recovery = results[check_idx].get("model_recovery", False)
                            
                            # If not in results, calculate on the fly using same logic as global section
                            if not check_model_recovery:
                                check_license_type_make_model = str(check_party_info.get("License_Type_From_Make_Model", "")).strip()
                                check_license_type_request = str(check_party_info.get("License_Type_From_Request", "")).strip()
                                
                                # Normalize values
                                if check_license_type_make_model.lower() in ["none", "nan", "null"]:
                                    check_license_type_make_model = ""
                                if check_license_type_request.lower() in ["none", "nan", "null"]:
                                    check_license_type_request = ""
                                
                                # Check model_recovery condition (same logic as global section)
                                check_make_model_valid = (check_license_type_make_model and 
                                                          check_license_type_make_model.lower() not in ["not identify", "not identified", "", "none", "nan", "null"] and
                                                          check_license_type_make_model.upper() != "ANY LICENSE")
                                check_request_is_none_or_empty = (not check_license_type_request or 
                                                                 check_license_type_request.lower() in ["not identify", "not identified", "", "none", "nan", "null"])
                                check_request_mismatch = (check_license_type_request and 
                                                         check_license_type_request.lower() not in ["not identify", "not identified", "", "none", "nan", "null"] and
                                                         check_license_type_make_model.upper() != check_license_type_request.upper())
                                check_model_recovery = check_make_model_valid and (check_request_is_none_or_empty or check_request_mismatch)
                            
                            # Check if model_recovery is True/TRUE/true (boolean or string)
                            if isinstance(check_model_recovery, str):
                                check_model_recovery_str = str(check_model_recovery).strip()
                                check_has_model_recovery = check_model_recovery_str.upper() in ["TRUE", "1", "YES", "Y"] or check_model_recovery_str in ["True", "true"]
                            else:
                                check_has_model_recovery = bool(check_model_recovery)
                            
                            if check_has_model_recovery:
                                other_party_with_model_recovery_field = True
                                other_model_recovery_field_party_info = {
                                    "idx": check_idx,
                                    "model_recovery": check_model_recovery,
                                    "liability": check_party_info.get("Liability", 0)
                                }
                            
                            # Break if both found (or continue to check all parties)
                            if other_party_with_recovery_field and other_party_with_model_recovery_field:
                                break
                        
                        # Note: other_tawuniya_with_model_recovery is already checked and set in the main loop above (lines 5475-5512)
                        # The check is done on-the-fly in the same loop where Recovery is checked
                        # This ensures model_recovery is calculated even if not yet in results dictionary
                        
                        # Determine if recovery validation should proceed
                        # NEW RULE: Recovery validation ONLY applies to Tawuniya parties with liability > 0 AND (Recovery=True OR model_recovery=True)
                        # Non-Tawuniya parties SKIP recovery validation entirely
                        current_liability = int(party_info.get("Liability", 0))
                        ...
                        if is_tawuniya_party:
                            ...
                            if current_liability > 0 and (current_has_recovery_field or current_has_model_recovery):
                                ...
                                if current_has_recovery_field:
                                    print(f"  ✅ Current party is Tawuniya with liability={current_liability}% AND Recovery=True - validating recovery conditions for upgrade")
                                elif current_has_model_recovery:
                                    print(f"  ✅ Current party is Tawuniya with liability={current_liability}% AND model_recovery=True - validating recovery conditions for upgrade")
                            # Also check if another Tawuniya party has Recovery=True/TRUE/true (any liability) AND current party has liability > 0
                            elif current_liability > 0 and other_tawuniya_with_recovery:
                                should_validate_recovery = True
                                if verbose_logging:
                                    print(f"  ✅ Current party is Tawuniya with liability={current_liability}% AND another Tawuniya party (Party {other_tawuniya_party_info['idx'] + 1}) has Recovery=True/TRUE/true - validating recovery conditions for upgrade")
                            # Also check if another Tawuniya party has model_recovery=True/TRUE/true (any liability) AND current party has liability > 0
                            elif current_liability > 0 and other_tawuniya_with_model_recovery:
                                should_validate_recovery = True
                                print(f"  ✅ Current party is Tawuniya with liability={current_liability}% AND another Tawuniya party (Party {other_tawuniya_model_recovery_party_info['idx'] + 1}) has model_recovery=True/TRUE/true - validating recovery conditions for upgrade (SAME AS Recovery)")
                            else:
                                if verbose_logging:
                                    if current_liability <= 0:
                                        print(f"  ℹ️ SKIPPING recovery validation: Current party is Tawuniya but liability={current_liability}% (must be > 0)")
                                    elif not current_has_recovery_field and not current_has_model_recovery:
                                        print(f"  ℹ️ SKIPPING recovery validation: Current party is Tawuniya but no Recovery=True or model_recovery=True found")
                        else:
                            # Non-Tawuniya party - Check exception: if another Tawuniya party has Recovery=True/TRUE/true with liability > 0
                            # EXCEPTION: Current party is non-Tawuniya with liability < 100% AND another Tawuniya party has Recovery=True/TRUE/true with liability > 0
                            if current_liability < 100 and other_tawuniya_with_recovery:
                                should_validate_recovery = True
                                print(f"  ✅ EXCEPTION: Current party is non-Tawuniya with liability={current_liability}% AND another Tawuniya party (Party {other_tawuniya_party_info['idx'] + 1}) has Recovery=True/TRUE/true with liability={other_tawuniya_party_info['liability']}% > 0 - validating recovery conditions for upgrade")
                            elif current_liability < 100 and other_tawuniya_with_model_recovery:
                                should_validate_recovery = True
                                print(f"  ✅ EXCEPTION: Current party is non-Tawuniya with liability={current_liability}% AND another Tawuniya party (Party {other_tawuniya_model_recovery_party_info['idx'] + 1}) has model_recovery=True/TRUE/true with liability={other_tawuniya_model_recovery_party_info['liability']}% > 0 - validating recovery conditions for upgrade (SAME AS Recovery)")
                            else:
                                # Non-Tawuniya party - SKIP recovery validation (no exception applies)
                                if verbose_logging:
                                    print(f"  ℹ️ SKIPPING recovery validation: Current party is non-Tawuniya (Insurance: {current_insurance}) - recovery validation only applies to Tawuniya parties or exception case")
                        
                        if not should_validate_recovery:
                            # Skip recovery validation
                            if verbose_logging:
                                print(f"  ℹ️ SKIPPING recovery validation: No Tawuniya party with Recovery=True found")
                        else:
                            # Proceed with recovery validation
                            if is_tawuniya_party:
                                if other_tawuniya_with_recovery:
                                    print(f"  ✅ Current party is Tawuniya AND found another Tawuniya party (Party {other_tawuniya_party_info['idx'] + 1}) with Recovery=True/TRUE/true (liability: {other_tawuniya_party_info['liability']}%)")
                                    print(f"  ✅ Validating recovery conditions for upgrade to ACCEPTED_WITH_RECOVERY")
                                elif other_tawuniya_with_model_recovery:
                                    print(f"  ✅ Current party is Tawuniya AND found another Tawuniya party (Party {other_tawuniya_model_recovery_party_info['idx'] + 1}) with model_recovery=True/TRUE/true (liability: {other_tawuniya_model_recovery_party_info['liability']}%)")
                                    print(f"  ✅ Validating recovery conditions for upgrade to ACCEPTED_WITH_RECOVERY (SAME AS Recovery)")
                                else:
                                    if verbose_logging:
                                        print(f"  ✅ Current party is insured with Tawuniya - validating recovery conditions for upgrade")
                            # Check exception for non-Tawuniya parties
                            if not is_tawuniya_party:
                                if other_tawuniya_with_recovery:
                                    print(f"  ✅ EXCEPTION: Found Tawuniya party (Party {other_tawuniya_party_info['idx'] + 1}) with Recovery=True/TRUE/true (liability: {other_tawuniya_party_info['liability']}% > 0)")
                                    print(f"  ✅ Current party (non-Tawuniya) has liability={current_liability}% < 100% - validating recovery conditions for upgrade to ACCEPTED_WITH_RECOVERY")
                                elif other_tawuniya_with_model_recovery:
                                    print(f"  ✅ EXCEPTION: Found Tawuniya party (Party {other_tawuniya_model_recovery_party_info['idx'] + 1}) with model_recovery=True/TRUE/true (liability: {other_tawuniya_model_recovery_party_info['liability']}% > 0)")
                                    print(f"  ✅ Current party (non-Tawuniya) has liability={current_liability}% < 100% - validating recovery conditions for upgrade to ACCEPTED_WITH_RECOVERY (SAME AS Recovery)")
                ...
                if not ocr_text_for_extraction_local and base64_files_path and case_number:
                    ...
                    try:
                        ...
                        # Only log errors for small batches or actual failures
                        if verbose_logging:
                ...
                # Debug: Print what will be saved to Excel (only for small batches)
                if verbose_logging:
                ...
                result_dict = {
                    ...
                    # Initialize model_recovery as False (will be updated globally if condition is met)
                    "model_recovery": False
                }
                results.append(result_dict)
            
            # ========== GLOBAL TAWUNIYA RULE: SUM OF LIABILITY = 0 ==========
            # CRITICAL RULE: If the sum of liability for parties insured under Tawuniya is 0,
            # then REJECT ALL parties in the accident (all claims should be rejected)
            # This check must be done AFTER all parties are processed (after the loop completes)
            # Only check once when we have processed all parties
            if len(parties) > 0:
                # Count party results (exclude error results)
                party_results_count = len([r for r in results if r.get("Party", 0) > 0 and r.get("Decision") != "ERROR"])
                
                # Only proceed if we've processed all parties
                if party_results_count == len(parties):
                    # Calculate sum of liability for Tawuniya parties only
                    tawuniya_liability_sum = 0
                    tawuniya_party_count = 0
                    tawuniya_party_indices = []
                    
                    for idx_check, party_decision_check in enumerate(parties):
                        # Get party info
                        party_raw_check = parties_data.get(idx_check, {})
                        if not party_raw_check and "party_info" in party_decision_check:
                            party_raw_check = party_decision_check.get("party_info", {})
                        
                        party_info_check = self.extract_party_info(party_raw_check)
                        
                        # Fallback from decision
                        if party_info_check.get("Liability") == 0 and "liability" in party_decision_check:
                            party_info_check["Liability"] = int(party_decision_check.get("liability", 0))
                        
                        party_liability = party_info_check.get("Liability", 0)
                        party_insurance = str(party_info_check.get("Insurance_Name", "")).strip()
                        party_ic_english = str(party_info_check.get("ICEnglishName", "")).strip()
                        
                        # Check if this party is insured with Tawuniya
                        is_tawuniya = self._is_tawuniya_insurance(party_insurance, party_ic_english)
                        
                        if is_tawuniya:
                            tawuniya_liability_sum += party_liability
                            tawuniya_party_count += 1
                            tawuniya_party_indices.append(idx_check)
                    
                    # If sum of Tawuniya liabilities = 0, REJECT ALL parties
                    if tawuniya_party_count > 0 and tawuniya_liability_sum == 0:
                        print(f"\n{'='*80}")
                        print(f"⚠️ CRITICAL: GLOBAL TAWUNIYA RULE - SUM OF LIABILITY = 0")
                        print(f"{'='*80}")
                        print(f"📊 Tawuniya Parties Analysis:")
                        print(f"   - Total Tawuniya parties: {tawuniya_party_count}")
                        print(f"   - Tawuniya party indices: {[idx + 1 for idx in tawuniya_party_indices]}")
                        print(f"   - Sum of Tawuniya liabilities: {tawuniya_liability_sum}%")
                        print(f"   - Total parties in accident: {len(parties)}")
                        print(f"\n⚠️ RULE: If sum of liability for Tawuniya parties = 0, ALL parties must be REJECTED")
                        print(f"⚠️ Applying global rejection to ALL {len(parties)} parties...")
                        
                        # Update ALL results to REJECTED
                        for result_idx, result in enumerate(results):
                            if result_idx < len(parties):  # Only update party results, not error results
                                old_decision = result.get("Decision", "")
                                if old_decision != "REJECTED":
                                    result["Decision"] = "REJECTED"
                                    # Update classification
                                    old_classification = result.get("Classification", "")
                                    result["Classification"] = f"Global Tawuniya Rule: Sum of Tawuniya liabilities = 0%"
                                    
                                    # Update description
                                    old_description = result.get("Description", "")
                                    tawuniya_parties_text = ", ".join([f"Party {idx + 1}" for idx in tawuniya_party_indices])
                                    new_description = f"Global Tawuniya Rule: Sum of liability for Tawuniya parties ({tawuniya_parties_text}) = 0%. All parties must be REJECTED. | {old_description}"
                                    result["Description"] = new_description[:5000]  # Limit length
                                    
                                    print(f"   ✓ Updated Party {result_idx + 1}: {old_decision} → REJECTED")
                        
                        print(f"✅ All {len(parties)} parties have been REJECTED due to Tawuniya liability sum = 0")
                        print(f"{'='*80}\n")
                    
                    # ========== GLOBAL RECOVERY UPGRADE RULE ==========
                    # CRITICAL RULE: If a Tawuniya party has Recovery=True/True/true OR model_recovery=True/TRUE/true
                    # AND liability > 0, then upgrade ALL OTHER parties in the same accident from ACCEPTED to ACCEPTED_WITH_RECOVERY
                    print(f"\n{'='*80}")
                    print(f"🔍 CHECKING GLOBAL RECOVERY UPGRADE RULE")
                    print(f"{'='*80}")
                    print(f"📋 Rule: If Tawuniya party has Recovery=True/True/true OR model_recovery=True/TRUE/true with liability > 0")
                    print(f"   → Upgrade ALL OTHER parties from ACCEPTED to ACCEPTED_WITH_RECOVERY")
                    
                    # Find Tawuniya parties with Recovery=True or model_recovery=True and liability > 0
                    tawuniya_with_recovery = []
                    for idx_check, party_decision_check in enumerate(parties):
                        # Only process party results (not error results)
                        if idx_check >= len(results):
                            continue
                        
                        result_check = results[idx_check]
                        ...
                        if is_tawuniya and party_liability > 0:
                            # Check Recovery field (accepts TRUE, True, true, 1, YES, Y)
                            # Prioritize results dictionary, fallback to party_info
                            recovery_field_result = str(result_check.get("Recovery", "")).strip()
                            recovery_field_info = str(party_info_check.get("Recovery", "")).strip()
                            recovery_field = recovery_field_result if recovery_field_result else recovery_field_info
                            recovery_field = recovery_field.upper()
                            has_recovery = recovery_field in ["TRUE", "1", "YES", "Y"]
                            
                            # Check model_recovery (accepts True, TRUE, true - boolean or string)
                            # First try results dictionary (might be set already)
                            model_recovery_value = result_check.get("model_recovery", False)
                            
                            # If not set in results, calculate on-the-fly using same logic as global model_recovery rule
                            if not model_recovery_value or (isinstance(model_recovery_value, bool) and not model_recovery_value) or (isinstance(model_recovery_value, str) and not model_recovery_value.strip()):
                                # Calculate model_recovery on-the-fly (same logic as global section)
                                license_type_make_model = str(result_check.get("License_Type_From_Make_Model", "")).strip()
                                license_type_request = str(result_check.get("License_Type_From_Request", "")).strip()
                                
                                # Fallback to party_info if not found in results
                                if not license_type_make_model:
                                    license_type_make_model = str(party_info_check.get("License_Type_From_Make_Model", "")).strip()
                                if not license_type_request:
                                    license_type_request = str(party_info_check.get("License_Type_From_Request", "")).strip()
                                
                                # Normalize values: handle "None", "nan", "null" as empty
                                if license_type_make_model.lower() in ["none", "nan", "null"]:
                                    license_type_make_model = ""
                                if license_type_request.lower() in ["none", "nan", "null"]:
                                    license_type_request = ""
                                
                                # Check if conditions are met for model_recovery
                                # Condition 1: License_Type_From_Make_Model must exist and not be "Any License"
                                make_model_valid = (license_type_make_model and 
                                                   license_type_make_model.lower() not in ["not identify", "not identified", "", "none", "nan", "null"] and
                                                   license_type_make_model.upper() != "ANY LICENSE")
                                
                                # Condition 2: License_Type_From_Request is None/empty OR doesn't match Make_Model
                                request_is_none_or_empty = (not license_type_request or 
                                                           license_type_request.lower() in ["not identify", "not identified", "", "none", "nan", "null"])
                                request_mismatch = (license_type_request and 
                                                   license_type_request.lower() not in ["not identify", "not identified", "", "none", "nan", "null"] and
                                                   license_type_make_model.upper() != license_type_request.upper())
                                
                                final_condition = make_model_valid and (request_is_none_or_empty or request_mismatch)
                                model_recovery_value = "True" if final_condition else False
                            
                            has_model_recovery = False
                            if isinstance(model_recovery_value, bool):
                                has_model_recovery = model_recovery_value is True
                            elif isinstance(model_recovery_value, str):
                                has_model_recovery = model_recovery_value.upper() in ["TRUE", "1", "YES", "Y"]
                            
                            if has_recovery or has_model_recovery:
                                tawuniya_with_recovery.append({
                                    "idx": idx_check,
                                    "party_num": idx_check + 1,
                                    "liability": party_liability,
                                    "recovery": has_recovery,
                                    "model_recovery": has_model_recovery
                                })
                                print(f"   ✅ Found Tawuniya Party {idx_check + 1}:")
                                print(f"      - Liability: {party_liability}% (> 0)")
                                print(f"      - Recovery: {has_recovery}")
                                print(f"      - model_recovery: {has_model_recovery}")
                    
                    # If any Tawuniya party has Recovery or model_recovery with liability > 0
                    if tawuniya_with_recovery:
                        print(f"\n⚠️ GLOBAL RECOVERY UPGRADE RULE APPLIES:")
                        print(f"   - Found {len(tawuniya_with_recovery)} Tawuniya party(ies) with Recovery/model_recovery=True and liability > 0")
                        tawuniya_party_nums = [p["party_num"] for p in tawuniya_with_recovery]
                        print(f"   - Tawuniya parties: {tawuniya_party_nums}")
                        print(f"   - Upgrading ALL OTHER parties from ACCEPTED to ACCEPTED_WITH_RECOVERY...")
                        
                        # Upgrade all OTHER parties (not the Tawuniya parties themselves)
                        upgraded_count = 0
                        for result_idx, result in enumerate(results):
                            if result_idx < len(parties):  # Only update party results, not error results
                                # Skip Tawuniya parties with recovery (they already have recovery)
                                is_tawuniya_party = result_idx in [p["idx"] for p in tawuniya_with_recovery]
                                if is_tawuniya_party:
                                    continue
                                
                                old_decision = result.get("Decision", "")
                                # Only upgrade ACCEPTED decisions
                                if old_decision == "ACCEPTED":
                                    result["Decision"] = "ACCEPTED_WITH_RECOVERY"
                                    
                                    # Update classification
                                    old_classification = result.get("Classification", "")
                                    tawuniya_parties_text = ", ".join([f"Party {p['party_num']}" for p in tawuniya_with_recovery])
                                    recovery_types = []
                                    for p in tawuniya_with_recovery:
                                        types = []
                                        if p["recovery"]:
                                            types.append("Recovery=True")
                                        if p["model_recovery"]:
                                            types.append("model_recovery=True")
                                        recovery_types.append(f"Party {p['party_num']} ({', '.join(types)})")
                                    
                                    new_classification = f"{old_classification} | Global Recovery Rule: Tawuniya party(ies) {tawuniya_parties_text} have Recovery/model_recovery=True with liability > 0%"
                                    result["Classification"] = new_classification[:5000]  # Limit length
                                    
                                    # Update description
                                    old_description = result.get("Description", "")
                                    new_description = f"Global Recovery Rule: Tawuniya party(ies) {tawuniya_parties_text} have Recovery/model_recovery=True with liability > 0%. All other parties upgraded to ACCEPTED_WITH_RECOVERY. ({', '.join(recovery_types)}) | {old_description}"
                                    result["Description"] = new_description[:5000]  # Limit length
                                    
                                    upgraded_count += 1
                                    print(f"   ✓ Upgraded Party {result_idx + 1}: ACCEPTED → ACCEPTED_WITH_RECOVERY")
                        
                        if upgraded_count > 0:
                            print(f"✅ Global Recovery Rule: Upgraded {upgraded_count} party(ies) from ACCEPTED to ACCEPTED_WITH_RECOVERY")
                        else:
                            print(f"ℹ️ No parties to upgrade (all parties already have ACCEPTED_WITH_RECOVERY or are REJECTED)")
                        print(f"{'='*80}\n")
                    else:
                        print(f"ℹ️ No Tawuniya party with Recovery/model_recovery=True and liability > 0 found")
                        print(f"   → Global Recovery Upgrade Rule does not apply")
                        print(f"{'='*80}\n")
                    
                    # ========== INDIVIDUAL MODEL RECOVERY RULE ==========
                    # CRITICAL RULE: Each party is evaluated individually for model_recovery:
                    # - License_Type_From_Make_Model != "Any License" AND not empty
                    # - License_Type_From_Request is None/empty OR License_Type_From_Request != License_Type_From_Make_Model
                    # Then set model_recovery = True for THAT specific party only
                    print(f"\n{'='*80}")
                    print(f"🔍 CHECKING MODEL RECOVERY CONDITIONS FOR EACH PARTY INDIVIDUALLY")
                    print(f"{'='*80}")
                    for idx_check, party_decision_check in enumerate(parties):
                        # Only process party results (not error results)
                        if idx_check >= len(results):
                        ...
                        # Get party info for fallback
                        party_raw_check = parties_data.get(idx_check, {})
                        if not party_raw_check and "party_info" in party_decision_check:
                            party_raw_check = party_decision_check.get("party_info", {})
                        party_info_check = self.extract_party_info(party_raw_check)
                        
                        # Get license type values - prioritize results dictionary (has final processed values)
                        license_type_make_model = str(result_check.get("License_Type_From_Make_Model", "")).strip()
                        license_type_request = str(result_check.get("License_Type_From_Request", "")).strip()
                        ...
                        print(f"\n📋 Party {idx_check + 1} Model Recovery Check:")
                        print(f"   - License_Type_From_Make_Model (from results): '{result_check.get('License_Type_From_Make_Model', '')}'")
                        print(f"   - License_Type_From_Request (from results): '{result_check.get('License_Type_From_Request', '')}'")
                        print(f"   - License_Type_From_Make_Model (final): '{license_type_make_model}'")
                        print(f"   - License_Type_From_Request (final): '{license_type_request}'")
                        
                        # Normalize values: handle "None", "nan", "null" as empty
                        if license_type_make_model.lower() in ["none", "nan", "null"]:
                            license_type_make_model = ""
                            print(f"   - Make_Model normalized to empty (was 'None/nan/null')")
                        if license_type_request.lower() in ["none", "nan", "null"]:
                            license_type_request = ""
                            print(f"   - Request normalized to empty (was 'None/nan/null')")
                        
                        # Check if conditions are met for model_recovery for THIS party
                        # Condition 1: License_Type_From_Make_Model must exist and not be "Any License"
                        make_model_valid = (license_type_make_model and 
                                           license_type_make_model.lower() not in ["not identify", "not identified", "", "none", "nan", "null"] and
                                           license_type_make_model.upper() != "ANY LICENSE")
                        
                        print(f"   - Make_Model valid check:")
                        print(f"     * Exists: {bool(license_type_make_model)}")
                        print(f"     * Not in invalid list: {license_type_make_model.lower() not in ['not identify', 'not identified', '', 'none', 'nan', 'null']}")
                        print(f"     * Not 'Any License': {license_type_make_model.upper() != 'ANY LICENSE'}")
                        print(f"     * Result: make_model_valid = {make_model_valid}")
                        ...
                        print(f"   - Request check:")
                        print(f"     * Request is None/Empty: {request_is_none_or_empty}")
                        if license_type_request:
                            print(f"     * Request exists, checking mismatch:")
                            print(f"       - Make_Model (upper): '{license_type_make_model.upper()}'")
                            print(f"       - Request (upper): '{license_type_request.upper()}'")
                            print(f"       - Match: {license_type_make_model.upper() == license_type_request.upper()}")
                            print(f"       - Mismatch: {request_mismatch}")
                        
                        final_condition = make_model_valid and (request_is_none_or_empty or request_mismatch)
                        print(f"   - Final condition: make_model_valid ({make_model_valid}) AND (request_is_none_or_empty ({request_is_none_or_empty}) OR request_mismatch ({request_mismatch}))")
                        print(f"   - Result: {final_condition}")
                        
                        # Set model_recovery for THIS party individually
                        # Save as string "True" (same format as Recovery field) for Excel compatibility
                        if final_condition:
                            result_check["model_recovery"] = "True"  # Save as string "True" (same as Recovery field format)
                            print(f"   ✅ Party {idx_check + 1}: model_recovery = True (License type mismatch detected)")
                            print(f"      - License_Type_From_Make_Model: '{license_type_make_model}'")
                            print(f"      - License_Type_From_Request: '{license_type_request if license_type_request else 'None/Empty'}'")
                            print(f"      - Saved as: 'True' (string, same format as Recovery field)")
                        else:
                            result_check["model_recovery"] = False  # Keep as False (boolean) when condition not met
                            print(f"   ❌ Party {idx_check + 1}: model_recovery = False (Conditions not met)")
    ...
    def process_excel_to_results(self, file_path: str,
        ...
        # Ensure we process all rows from start_row to end_row
        rows_to_process = list(range(start_row, end_row))
        total_rows = len(rows_to_process)
        
        # Verify rows to process
        if total_rows == 0:
            raise ValueError(f"No rows to process! start_row={start_row}, end_row={end_row}, total_df_rows={len(df)}")
        
        # Log which rows will be processed (only for small batches)
        if total_rows < 50:
            print(f"✓ Will process rows {start_row + 1} to {end_row} (total: {total_rows} rows)")
        else:
            print(f"✓ Will process {total_rows} rows (from row {start_row + 1} to {end_row})")
        ...
        # OPTIMIZATION: Deduplicate cases - process each unique case only once
        # Step 1: Extract case numbers from all rows to identify unique cases
        case_to_rows = {}  # Maps case_number -> list of (idx, row_num, claim_data)
        case_extraction_errors = []  # Rows where we couldn't extract case number
        
        print(f"\n🔍 Step 1: Identifying unique cases from {total_rows} rows...")
        for idx, row_num in enumerate(rows_to_process):
            # Verify row_num is valid
            if row_num >= len(df):
                print(f"⚠️ Warning: Row {row_num + 1} is out of range (Excel has {len(df)} rows), skipping")
                continue
            
            claim_data = df.at[row_num, request_column]
            
            # Try to extract case number quickly (without full processing)
            try:
                case_number = None
                claim_str = str(claim_data).strip()
                
                # Quick extraction from JSON/XML
                if claim_str.startswith("{"):
                    import json
                    json_data = json.loads(claim_str)
                    # Try common paths for case number
                    case_number = (
                        json_data.get("case_number") or
                        json_data.get("caseNumber") or
                        json_data.get("Case_Number") or
                        json_data.get("Case_Info", {}).get("case_number") or
                        json_data.get("Case_Info", {}).get("Case_Number") or
                        json_data.get("Accident_info", {}).get("case_number") or
                        json_data.get("Accident_info", {}).get("Case_Number") or
                        None
                    )
                elif claim_str.startswith("<"):
                    # Quick XML extraction
                    from xml.etree import ElementTree as ET
                    try:
                        root = ET.fromstring(claim_str)
                        # Try common XML paths
                        case_elem = root.find(".//case_number") or root.find(".//caseNumber") or root.find(".//Case_Number")
                ...
                if case_number:
                    ...
                    if case_number not in case_to_rows:
                        case_to_rows[case_number] = []
                    case_to_rows[case_number].append((idx, row_num, claim_data))
                else:
                    # Can't extract case number - will process separately
                    case_extraction_errors.append((idx, row_num, claim_data))
            except Exception as e:
                # Error extracting case - will process separately
                case_extraction_errors.append((idx, row_num, claim_data))
        
        # Report deduplication results
        unique_cases = len(case_to_rows)
        total_rows_with_cases = sum(len(rows) for rows in case_to_rows.values())
        rows_without_cases = len(case_extraction_errors)
        
        print(f"✓ Found {unique_cases} unique case(s) across {total_rows_with_cases} row(s)")
        if rows_without_cases > 0:
            print(f"⚠️ {rows_without_cases} row(s) couldn't extract case number - will process separately")
        
        # Step 2: Prepare tasks - one task per unique case (not per row)
        tasks = []
        case_row_mapping = {}  # Maps (case_number, party_index) -> list of (idx, row_num) for result mapping
        
        # Add one task per unique case
        for case_number, rows_list in case_to_rows.items():
            # Use the first row's claim_data for processing (all rows have same case)
            first_idx, first_row_num, first_claim_data = rows_list[0]
            tasks.append((first_idx, first_row_num, first_claim_data, case_number, rows_list))
        
        # Add tasks for rows where we couldn't extract case number
        for idx, row_num, claim_data in case_extraction_errors:
            tasks.append((idx, row_num, claim_data, None, [(idx, row_num, claim_data)]))
        
        # Log task preparation
        if total_rows < 50:
            print(f"✓ Prepared {len(tasks)} unique case(s) to process (from {total_rows} rows)")
        else:
            print(f"✓ Optimized: Processing {len(tasks)} unique case(s) instead of {total_rows} rows ({total_rows - len(tasks)} duplicates skipped)")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            # Submit ALL tasks to executor - one per unique case
            # Use explicit loop to avoid any closure issues with variables
            future_to_task = {}
            for task_data in tasks:
                if len(task_data) == 5:
                    # Task with case deduplication
                    task_idx, row_num, claim_data, case_number, rows_list = task_data
                    # Store rows_list for result mapping
                    ...
                    future_to_task[future] = (task_idx, row_num, rows_list)
                else:
                    # Task without case number (fallback - old format)
                    task_idx, row_num, claim_data = task_data[:3]
                    rows_list = [(task_idx, row_num, claim_data)]
                    future = executor.submit(
                        self._process_single_row, 
                        row_num, 
                        claim_data, 
            ...
            if len(future_to_task) != len(tasks):
                print(f"⚠️ Warning: Submitted {len(future_to_task)} futures but expected {len(tasks)} tasks")
            
            # Collect results as they complete (optimized for large batches)
            completed = 0
            last_progress_time = start_time
            
            # Track processed rows to ensure all are processed
            processed_rows = set()
            
            # MEMORY OPTIMIZATION: Incremental saving every N records
            save_batch_size = 10  # Save every 10 records to avoid memory issues
            batch_results = []  # Accumulate results in batches
            saved_count = 0  # Track how many records have been saved
            
            # Prepare output file path early
            if output_file is None:
                base_name = os.path.splitext(os.path.basename(file_path))[0]
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                output_file = f"{base_name}_results_{timestamp}.xlsx"
            
            # Initialize Excel file with headers (will be created on first save)
            excel_file_initialized = False
            
            for future in as_completed(future_to_task):
                task_info = future_to_task[future]
                if len(task_info) == 3:
                    idx, row_num, rows_list = task_info
                else:
                    # Fallback for old format
                    idx, row_num = task_info[:2]
                    rows_list = [(idx, row_num, None)]
                
                # Verify we're processing different cases (not stuck on first case)
                if row_num in processed_rows:
                    if total_rows < 50:
                        print(f"⚠️ Warning: Row {row_num + 1} processed multiple times! This indicates a bug.")
                processed_rows.add(row_num)
                
                try:
                    row_results = future.result()
                    
                    # Map results to all rows that share this case
                    new_results = []
                    with results_lock:
                        # If this case appears in multiple rows, duplicate results for each row
                        if len(rows_list) > 1:
                            # This case appears in multiple rows - duplicate results for each
                            # Each row should get the same party results
                            for result_row_idx, result_row_num, _ in rows_list:
                                # Add all party results for this row
                                for result in row_results:
                                    # Create a copy of result for this specific row
                                    result_copy = result.copy()
                                    all_results.append(result_copy)
                                    new_results.append(result_copy)
                            if total_rows < 50:
                                print(f"  ✓ Mapped case results to {len(rows_list)} row(s)")
                        else:
                            # Single row - add results as normal
                            all_results.extend(row_results)
                            new_results.extend(row_results)
                    
                    # Add to batch for incremental saving
                    batch_results.extend(new_results)
                    completed += len(rows_list)  # Count all rows that got results
                    
                    # MEMORY OPTIMIZATION: Save every N records to avoid memory buildup
                    if len(batch_results) >= save_batch_size:
                        try:
                            self._save_batch_to_excel(batch_results, output_file, excel_file_initialized, 
                                                     daa_columns, file_path, start_row, end_row)
                            saved_count += len(batch_results)
                            batch_results = []  # Clear batch to free memory
                            excel_file_initialized = True
                            if total_rows >= 50:
                                print(f"  💾 Saved batch: {saved_count} records written to disk")
                        except Exception as e:
                            print(f"  ⚠️ Error saving batch: {str(e)[:200]}")
                    ...
                    if total_rows >= 50:
                        if completed % max(1, total_rows // 10) == 0 or (current_time - last_progress_time).total_seconds() >= 10:
                            ...
                            print(f"  ⚡ Progress: {completed}/{total_rows} ({completed*100//total_rows}%) | "
                                  f"Speed: {rate:.1f} rows/sec | ETA: {eta_minutes:.1f} min | Rows: {sorted(list(processed_rows))[:10]}...")
                            last_progress_time = current_time
                    else:
                        # For smaller batches, show every completion (only if < 20 rows)
                        if total_rows < 20:
                            print(f"  ✓ Completed row {row_num + 1} ({completed}/{total_rows} total)")
        ...
        if len(results_df) == 0:
            ...
            if 'completed' in locals():
                print(f"⚠️ Processed {completed} rows but got 0 results. Check for errors above.")
            return results_df
        
        # Verify all rows were processed (only warn if significant discrepancy)
        if 'completed' in locals() and completed < total_rows:
            missing = total_rows - completed
            if missing > 0:
                print(f"\n⚠️ Warning: Only processed {completed} out of {total_rows} rows ({missing} rows missing)")
                if 'processed_rows' in locals():
                    expected_rows = set(range(start_row, end_row))
                    missing_rows = expected_rows - processed_rows
                    if missing_rows:
                        print(f"⚠️ Missing row numbers: {sorted(list(missing_rows))[:20]}")
        elif 'completed' in locals() and completed == total_rows:
            if total_rows < 50:
        ...
        if daa_columns and len(daa_columns) > 0:
            ...
            # PERFORMANCE OPTIMIZATION: Build lookup dictionary first, then use vectorized operations
            matched_count = 0
            import re
            
            # Build Case_Number to DAA data lookup (O(1) access)
            case_number_to_daa = {}
            for excel_row_idx, daa_data in daa_columns.items():
                case_match = daa_data.get('Case_Number_Match', '')
                if case_match:
                    case_key = str(case_match).strip()
                    if case_key:
                        case_number_to_daa[case_key] = daa_data
            
            # Use itertuples() instead of iterrows() - much faster
            for row_tuple in results_df.itertuples():
                idx = row_tuple.Index
                case_number = str(getattr(row_tuple, 'Case_Number', '')).strip()
                ...
                # Try matching by Case_Number first (O(1) lookup)
                if case_number and case_number in case_number_to_daa:
                    daa_data = case_number_to_daa[case_number]
            ...
            # PERFORMANCE OPTIMIZATION: Use vectorized operations instead of apply()
            # Create new "Suspected Fraud" column based on isDAA (vectorized)
            isDAA_series = results_df['isDAA'].astype(str).str.strip().str.upper()
            results_df['Suspected_Fraud'] = None
            # Vectorized condition check
            fraud_mask = isDAA_series.isin(['TRUE', '1', 'YES', 'Y', 'T'])
            results_df.loc[fraud_mask, 'Suspected_Fraud'] = 'Suspected Fraud'
            # Handle NaN values
            results_df.loc[results_df['isDAA'].isna(), 'Suspected_Fraud'] = None
            ...
            # But check if they exist from Request data extraction
            if 'isDAA' not in results_df.columns:
                results_df['isDAA'] = None
            if 'Suspect_as_Fraud' not in results_df.columns:
                results_df['Suspect_as_Fraud'] = None
            ...
            # PERFORMANCE OPTIMIZATION: Use vectorized operations instead of apply()
            # Always create Suspected_Fraud column based on isDAA (even if from Request data)
            isDAA_series = results_df['isDAA'].astype(str).str.strip().str.upper()
            results_df['Suspected_Fraud'] = None
            # Vectorized condition check
            fraud_mask = isDAA_series.isin(['TRUE', '1', 'YES', 'Y', 'T'])
            ...
            # Only log for small batches
            if total_rows < 50:
                print(f"✓ Created 'Suspected_Fraud' column based on isDAA values (from Request data)")
        
        # Performance summary (always show, but brief for large batches)
        if total_rows >= 50:
            # Brief summary for large batches
            avg_time_per_row = total_time / total_rows if total_rows > 0 else 0
            print(f"\n⚡ Completed: {total_rows} rows, {len(results_df)} parties in {total_time:.1f}s ({total_time/60:.1f} min) | Avg: {avg_time_per_row:.2f}s/row")
        else:
            # Detailed summary for small batches
            print(f"\n{'=' * 60}")
        ...
        # Only show detailed stats for small batches
        if total_rows < 50:
        ...
        column_order = [
            ...
            "isDAA",  # DAA columns from original Excel - placed before Decision
            "Suspect_as_Fraud",
            "DaaReasonEnglish",
            "Suspected_Fraud",  # New column: "Suspected Fraud" if isDAA is TRUE, else null
            "model_recovery",  # New column: True if license type mismatch detected for any party in accident
            "Decision", 
            "Classification", 
            "Description"
        ]
        ...
        # Check if incremental CSV file exists (from batch saves)
        csv_temp_file = output_file.replace('.xlsx', '_temp.csv')
        if os.path.exists(csv_temp_file):
            # Load all data from CSV and convert to Excel
            try:
                print(f"\n💾 Converting incremental CSV to Excel...")
                csv_df = pd.read_csv(csv_temp_file, encoding='utf-8-sig')
                # Merge with any remaining results
                if len(results_df) > 0:
                    # Combine CSV data with final results
                    final_df = pd.concat([csv_df, results_df], ignore_index=True)
                else:
                    final_df = csv_df
                ...
                # Brief message for large batches, detailed for small
                if total_rows >= 50:
                    print(f"✓ Saved: {output_file} ({len(final_df)} rows)")
                else:
                    print(f"\n{'=' * 60}")
                    print(f"✓ Results saved to: {output_file}")
                    print(f"✓ Total parties processed: {len(final_df)}")
                    print(f"{'=' * 60}")
                
                return final_df
            except Exception as e:
                print(f"  ⚠️ Error converting CSV to Excel: {str(e)[:200]}")
                # Fall through to normal save
        
        # Normal save (no incremental CSV)
        try:
            ...
            try:
                ...
                print(f"  ✗ Error saving CSV: {str(e2)}")
        
        return results_df
    
    def _save_batch_to_excel(self, batch_results: List[Dict], output_file: str, 
                             file_initialized: bool, daa_columns: Dict, 
                             original_file_path: str, start_row: int, end_row: int):
        """
        Save a batch of results incrementally to Excel file every 10 records.
        Appends directly to Excel file to avoid memory issues.
        
        Args:
            batch_results: List of result dictionaries to save
            output_file: Path to output Excel file
            file_initialized: Whether the Excel file already exists
            daa_columns: DAA column data from original Excel
        ...
        try:
            ...
            # Apply DAA merging if needed (simplified for batches)
            if daa_columns and len(daa_columns) > 0:
                # Initialize DAA columns if not present
                for col in ['isDAA', 'Suspect_as_Fraud', 'DaaReasonEnglish']:
                    if col not in batch_df.columns:
                        batch_df[col] = None
                
                # Quick DAA merge (simplified for batches)
                import re
                case_number_to_daa = {}
                for excel_row_idx, daa_data in daa_columns.items():
                    case_match = daa_data.get('Case_Number_Match', '')
                ...
                # Merge DAA data for this batch
                for idx, row in batch_df.iterrows():
                    case_num = str(row.get('Case_Number', '')).strip()
                    if case_num and case_num in case_number_to_daa:
                        daa_data = case_number_to_daa[case_num]
                        # Only update if current value is None/empty
                        if pd.isna(batch_df.at[idx, 'isDAA']) or str(batch_df.at[idx, 'isDAA']).strip() == '':
                            batch_df.at[idx, 'isDAA'] = daa_data.get('isDAA')
                        if pd.isna(batch_df.at[idx, 'Suspect_as_Fraud']) or str(batch_df.at[idx, 'Suspect_as_Fraud']).strip() == '':
                            batch_df.at[idx, 'Suspect_as_Fraud'] = daa_data.get('Suspect_as_Fraud')
                        if pd.isna(batch_df.at[idx, 'DaaReasonEnglish']) or str(batch_df.at[idx, 'DaaReasonEnglish']).strip() == '':
                            batch_df.at[idx, 'DaaReasonEnglish'] = daa_data.get('DaaReasonEnglish')
            
            # Create Suspected_Fraud column
            if 'isDAA' in batch_df.columns:
                isDAA_series = batch_df['isDAA'].astype(str).str.strip().str.upper()
                batch_df['Suspected_Fraud'] = None
                fraud_mask = isDAA_series.isin(['TRUE', '1', 'YES', 'Y', 'T'])
                batch_df.loc[fraud_mask, 'Suspected_Fraud'] = 'Suspected Fraud'
                batch_df.loc[batch_df['isDAA'].isna(), 'Suspected_Fraud'] = None
            
            # Reorder columns (same as final output)
            column_order = [
                "Case_Number", "Party", "Party_ID", "Party_Name",
                "Insurance_Name", "ICEnglishName", "Liability", 
                "Vehicle_Serial", "VehicleOwnerId",
                "License_Type_From_Request", "Recovery",
                "License_Expiry_Date", "Upload_Date", 
                "License_Expiry_Last_Updated", "Accident_Date",
                "carMake", "carModel", "License_Type_From_Make_Model",
                "Full_Analysis", "Full_Analysis_English",
                "isDAA", "Suspect_as_Fraud", "DaaReasonEnglish", "Suspected_Fraud",
                "model_recovery",
                "Decision", "Classification", "Description"
            ]
            available_columns = [col for col in column_order if col in batch_df.columns]
            batch_df = batch_df[available_columns]
            
            # Append to Excel file directly
            if file_initialized and os.path.exists(output_file):
                # Append mode - read existing file and append new rows
                try:
                    from openpyxl import load_workbook
                    book = load_workbook(output_file)
                    sheet = book.active
                    
                    # Find next empty row (skip header row)
                    next_row = sheet.max_row + 1
                    
                    # Write batch data starting from next row
                    for r_idx, row_data in enumerate(batch_df.itertuples(index=False), start=next_row):
                        for c_idx, value in enumerate(row_data):
                            # Handle None values
                            if pd.isna(value) or value is None:
                                value = ""
                            # Truncate very long strings to avoid Excel errors
                    ...
                    print(f"  ⚠️ Error appending to Excel, creating new file: {str(e)[:100]}")
                    file_initialized = False
            
            if not file_initialized:
                # Create new file with headers
                batch_df.to_excel(output_file, index=False, engine='openpyxl')
                file_initialized = True
            
            return file_initialized
                
        except Exception as e:
            print(f"  ⚠️ Error saving batch to Excel: {str(e)[:200]}")
            # Try CSV as fallback
            try:
                csv_file = output_file.replace('.xlsx', '_fallback.csv')
                batch_df.to_csv(csv_file, mode='a', header=not file_initialized, index=False, encoding='utf-8-sig')
