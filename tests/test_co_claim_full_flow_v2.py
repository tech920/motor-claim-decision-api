...
# Fix the test script to properly mock the requests used in ClaimProcessor.call_ollama
# The previous test failed because ClaimProcessor.call_ollama uses 'requests.Session()' which wasn't fully mocked
# We need to mock requests.Session().post
            ...
            with patch('MotorclaimdecisionlinuxCO.claim_processor_api.jsonify', lambda x: x):
                with patch('MotorclaimdecisionlinuxCO.claim_processor_api.co_ocr_license_processor') as mock_ocr:
                    ...
                    try:
                        ...
                        if parties and parties[0].get('Decision') == 'ACCEPTED_WITH_RECOVERY':
                             print("\n✅ SUCCESS: Full flow verified - Translation -> Decision -> Response")
                        else:
                             print("\n❌ FAILURE: Unexpected decision result")
